...
// ADDED IN TODAY'S LECTURE STEP 5
	...
	btVector3 GamePhysicsTestBed::GetPickingRay(int x, int y) {
	...
	void GamePhysicsTestBed::ShootBox(const btVector3 &direction) {
	...
	bool GamePhysicsTestBed::Raycast(const btVector3 &startPosition, const btVector3 &direction, RayResult &output) {
		if (!m_pWorld) 
			return false;
	...
	void GamePhysicsTestBed::DestroyGameObject(btRigidBody* pBody) {
...
// ADDED TO TODAY'S LECTURE STEP 12
	void GamePhysicsTestBed::RemovePickingConstraint() {
		// exit in erroneous situations
